	...
	if #impurities ~= 1 then
		if level == 0 then
			...
			res[#res + 1] = {
				...
				amount_min = avg - 1,
				amount_max = avg + 1,
				probability = (level + 1) / count * 4 / avg
			}
			for _, imp in pairs(impurities) do
				if imp ~= focus then
					res[#res + 1] = {
...
local get_omnimatter_split = function(tier, focus, level)
	...
	if source_count == 0 or (source_count == 1 and not focus) then
		return {
			{
				result_round(
					{name = "stone-crushed", amount = 10 - 4 / (omni.impure_levels_per_tier - (level or 0) + 1), type = "item"}
				),
				result_round(
					{name = focus or aligned_ores[1], amount = 4 / (omni.impure_levels_per_tier - (level or 0) + 1), type = "item"}
				)
			}
		}
	end
		...
		for _, ore_mix in pairs(ores) do
			for i, ore in pairs(ore_mix) do
				if ore["name"] == focus then
					...
					local prnt = {}
					for k, v in pairs(focus_ores) do
						prnt[#prnt + 1] = v.name
					end
					print("Focused recipe:" .. table.concat(prnt, ", "))
					return {focus_ores}
				end
			end
		end
	...
	print("Recipe:" .. table.concat(prnt, ", "))
	return ores
end
...
for i, tier in pairs(omnisource) do
	for ore_name, ore in pairs(tier) do
		...
		local cost =
			OmniGen:create():setInputAmount(12):setYield(item):setIngredients("omnite"):setWaste("stone-crushed"):yieldQuant(
			extraction_value
		):wasteQuant(
		...
		local pure_ore =
			RecChain:create("omnimatter", "extraction-" .. item):setLocName(
			"recipe-name.pure-omnitraction",
			{"item-name." .. item}
		):setIngredients("omnite"):setIcons(item):setIngredients(cost:ingredients()):setResults(cost:results()):setEnabled(
			false
		):setCategory("omnite-extraction"):setSubgroup("omni-pure"):setMain(item):setLevel(3 * omni.pure_levels_per_tier):setEnergy(
			function(levels, grade)
				return 5 * (math.floor((grade - 1 + (tier_int - 1) / 2) / levels) + 1)
			end
		):setTechIcon(generate_pure_icon(ore)):setTechCost(
			function(levels, grade)
...
for _, ore_tiers in pairs(omnisource) do
	...
	for i, split in pairs(base_split) do
		...
		local base_impure_ore =
			RecGen:create("omnimatter", "omnirec-base-" .. i .. "-extraction-" .. t):setLocName("base-impure", {t}):setIngredients(
			{name = "omnite", type = "item", amount = 10}
		):setSubgroup("omni-impure-basic"):setEnergy(5 * (math.floor(t / 2 + 0.5))):setTechUpgrade(t > 1):setTechCost(tc):setEnabled(
			false
		...
		if t == 1 then
			base_impure_ore:setCategory("omnite-extraction-both"):setTechPrereq(nil):setTechName("base-impure-extraction"):setTechLocName(
				"base-omnitraction"
			)
		else
			base_impure_ore:setCategory("omnite-extraction"):setTechName("omnitractor-electric-" .. (t - 1))
		end
	...
	for _, ore in pairs(ore_tiers) do
		...
		for i, sp in pairs(level_splits) do
			for j, r in pairs(sp) do
				local focused_ore =
					RecGen:create("omnimatter", "omnirec-focus-" .. j .. "-" .. ore.name .. "-" .. ord[i]):setLocName(
					"impure-omnitraction",
					{"item-name." .. ore.name}
				):setIngredients({name = "omnite", type = "item", amount = 10}):setSubgroup("omni-impure"):setEnergy(
					5 * (math.floor(t / 2 + 0.5))
				):setIcons("omnite"):setEnabled(false):addIcon(
					{
						icon = ore.name,
						scale = 0.4375,
						shift = {10, 10}
					}
				):addBlankIcon():setTechName("omnitech-focused-extraction-" .. ore.name .. "-" .. i):setTechCost(25 * i * t):setTechLocName(
					"impure-omnitraction",
					"item-name." .. ore.name,
					i
				):setTechPacks(math.max(1, t)):--setTechIcon(ore.mod or "omnimatter","omnimatter"):--"impure_"..ore.name):
				setTechIcon(generate_impure_icon(ore)):marathon()

				if #sp > 1 then
					focused_ore:addIcon(
						{
							icon = "__omnilib__/graphics/icons/small/num_" .. j .. ".png",
							scale = 0.6,
							shift = {-10, -10}
						}
					)
